    ...
    <form action="{{ route('tags.update', $tag->id) }}" method="post">
        ...
        <div class="card  mb-4">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h2 class="card-title mb-3">{{__('Tag edit')}}   </h2>
            </div>
            <div class="card-body">
                <div class="row mb-5 ">
                    <div class="col-md-12">
                        <label class="form-label">{{__('Name')}} {{$tag->lang->name}}
                                <span style="color: red;">*</span>
                            </label>
                        <input type="text" class="form-control" placeholder="{{__('Name')}}" name="name"  required  value="{{$tag->name}}" >
{{--
                            ...
                            @foreach($langs as $key=>$lang)
                                ...
                                <div class="tab-pane fade  @if($key==0) show active @endif " id="{{$lang->slug}}" role="tabpanel" aria-labelledby="{{$lang->slug}}-tab">
                                    <label class="form-label">{{__('Name')}} {{__($lang->slug)}} @if($reg)
                                            <span style="color: red;">*</span>
                                        @endif</label>
                                    <input type="text" class="form-control" placeholder="{{__('Name')}}" name="{{$slug}}" @if($reg) required @endif value="{{$value}}" >
                                </div>
                            @endforeach
                        </div>
                         --}}
